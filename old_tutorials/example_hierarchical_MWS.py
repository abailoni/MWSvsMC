import sys
sys.path.append("/home/mlcvss18_3/python_pkgs/nifty/python")
...
if __name__ == '__main__':
    ...
    crop_slice = (slice(None), slice(None, 10), slice(None, 200), slice(None, 200))
    ...
    # TODO: REPLACE OR COMPARE WITH YOUR IMPLEMENTATION
    final_node_labels, runtime = MWS(graph, edge_weights, is_edge_attractive)
    ...
    print("Clustering took {}s. Final MC energy: {}".format(runtime, energy))
    ...
    utils.plot_segm(ax, final_segm, z_slice=5, background=raw)
    f.savefig(os.path.join(plots_path, 'MWS_final.pdf'), format='pdf')

    # Save some data:
    vigra.writeHDF5(final_segm.astype('uint32'), os.path.join(save_path, "final_segm_MWS}.h5"), 'data')
