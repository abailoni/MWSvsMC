    ...
    def get_list_of_runs(self, path):
        ...
        for item in os.listdir(path):
            if os.path.isfile(os.path.join(path, item)):
                ...
                if len(filename.split("_")) not in [4]:
                    continue
...
class PixelEmbeddingTrainSamples(CremiExperiment):
    def __init__(self, *super_args, **super_kwargs):
        ...
        self.fixed_kwargs.update({
            ...
            # "experiment_name": "FullTrain_dice",
            # "experiment_name": "FullTrain_emb",
            "experiment_name": "fixed_IoU_2_19_19",
            # "experiment_name": "FullTrain_SOA",
            "local_attraction": False,
            # "additional_model_keys": ["debug_postproc", "invertAffs"],
            # "additional_model_keys": ["debug_postproc", "simple_WSDT", "invertAffs", "setWSDTLocalOffsets"],
            # "additional_model_keys": ["debug_postproc", "boundaryPixels_IoU", "invertAffs", "setWSDTLocalOffsets"],
            "additional_model_keys": ["debug_postproc", "simple_WSDT", "crop_padded_affs"],
            # "additional_model_keys": ["debug_postproc", "stride10", "crop_padded_affs"],
            # "additional_model_keys": ["debug_postproc", "boundaryPixels_IoU"],
            # "compute_scores": True,
    ...
    def collect_scores(self, project_directory):
        ...
        new_table_entrance = []
        for j, key in enumerate(string_keys):
            new_table_entrance.append(key[-1])
        ...
        for config, json_file in zip(config_list, json_file_list):
            new_table_entrance = []
            for j, key in enumerate(string_keys):
                new_table_entrance.append(str(return_recursive_key_in_dict(config, key)))
            for j, key in enumerate(keys_to_collect):
                value = return_recursive_key_in_dict(config, key)
                new_table_entrance.append("{0:.{prec}{type}}".format(value, prec=nb_flt_digits[j],
                                                                     type=nb_formats[j]))
            collected_results.append(new_table_entrance)

        collected_results = np.array(collected_results)
        collected_results[1:] = collected_results[1:][collected_results[1:, sorting_column_idx].argsort()]
